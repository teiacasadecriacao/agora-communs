<?php
$pressed=-1;
require("cabecalho.php");
?>
<h1>Sobre</h1>
    <div class="wrapper">
        <p style="color:white">
            Por hora o que planejamos está como segue. As pautas propostas (segundo link do menu)
            podem ser promovias a pautas ativas (primeiro link do menu). As pautas ativas
            primeiramente ficam abertas para postagens, estas
            podem ser de um dos dois tipos: discussão/debate ou deliberação/encaminhamento.
            Qualquer postagem fica aberta para comentários e estes podem ser feitos
            também por usuários não credenciados (habilitados por usuários administradores).
        </p>
        <p style="color:white">
            Em algum momento uma pauta deixa de estar aberta e é iniciada uma votação nas
            deliberações. Cada usuário pode votar uma única vez em cada deliberação.
            Pode-se ver quem votou a favor ou contra. Pode-se ainda postar comentários
            em quaisquer postagens, sejam deliberações ou não.
        </p>
        <p style="color:white">
            Por fim, a pauta encerra o período de votações e então a pauta fica disponivel
            para consulta tanto dos conteúdos postados quanto das votações. Nesta etapa
            a pauta fica acessível a partir do terceiro link do menu. Sempre é possível
            postar comentários às postagens, mesmo com a pauta encerrada.
        </p>
        <p style="color:white">
            Esquema de inserção de conteúdos:
            Pauta - Postagem - Comentários.<br />
            Uma pauta pode ter várias postagens. Uma postagem pode ter vários comentários.
        </p>
        <p style="color:white">
            Esquema temporal de pautas:
            Pauta proposta - Pauta aberta para postagens - Pauta em votação - Pauta encerrada.
        </p>

        <p style="color:red;font-size:12px;text-align:center;">
            /\/\/\/\/-------/\/\/\/\/ h4csw4r3 de delibera&ccedil;&atilde;o _o_o_ oOo _o_o_<br />
            Um hacksware é um programa feito sem proposta comercial, sem pompa, sem querer
            convencer ninguém ou enfiar algo guela abaixo. Também outra coisa. Aproveite.
        </p>
    </div>
</body>
</html>
